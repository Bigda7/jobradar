...
DEFAULT_API_BASE_URL = "https://api.ashbyhq.com"
...
class AshbySource(BaseSource):
    ...
    def normalize(self, raw_listing: RawListing) -> NormalizedOpportunity:
        ...
        if not _is_remote(payload):
            raise AtsSourceError("Ashby listing is not strictly remote.")
        description = optional_string(payload.get("descriptionPlain"))
        if description is None:
            description = html_to_text(
                required_string(payload.get("descriptionHtml"), "descriptionHtml")
            )
        salary = _salary(payload.get("compensation"))
        return NormalizedOpportunity(
            kind=self.opportunity_kind,
            title=required_string(payload.get("title"), "title"),
            company=company_name(payload),
            description=description,
            location_text=optional_string(payload.get("location")) or "Remote",
            work_mode=WorkMode.REMOTE,
            employment_type=employment_type(payload.get("employmentType")),
...
def _is_remote(payload: Mapping[str, Any]) -> bool:
    ...
    if workplace_type and workplace_type != "remote":
        return False
    return payload.get("isRemote") is True or workplace_type == "remote"
...
def _salary(value: Any) -> AtsSalary | None:
    ...
    for component in components:
        if not isinstance(component, Mapping):
            continue
        if normalize_text(optional_string(component.get("compensationType"))) != "salary":
            continue
        minimum = decimal_amount(component.get("minValue"))
        maximum = decimal_amount(component.get("maxValue"))
        currency = optional_string(component.get("currencyCode"))
        ...
        return AtsSalary(
            ...
            period=salary_period(component.get("interval")),
        )
    return None
